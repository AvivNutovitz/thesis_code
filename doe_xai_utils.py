...
models_and_names = {'LR': LogisticRegression(random_state=random.seed(seed)),
...
def get_scores_by_adding_selected_features(x_train, y_train, x_test, y_test, order_features, data_set_name, model_name=None, method='shap', max_features=25, plot=True):
    ...
    predictions, features_in_group = _add_selected_features_iteratively_train_and_predict(x_train, y_train,
                                                                                          x_test, order_features, model_name, max_features)
    ...
    scores['mutual_info'] = pd.Series([np.round(mutual_info_classif(x_train[features], y_train, n_neighbors=len(features),
                                                                    random_state=seed).sum(), 3) for features in features_in_group]).cummax()
...
def reduce_multicollinearity(X_train, X_test, data_set_name):
    vif = pd.DataFrame()
...
def train_model_get_score_by_model_name(model, model_name, new_X_train, y_train, new_X_test, y_test, replication_index=0):
    if model_name == 'CNN' or model_name == 'DNN':
        print()
